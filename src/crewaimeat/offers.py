...
# The authored per-crew offers used to be an 858-line dict literal right here — one more central list
# nothing forced you to update, and on 2026-08-22 it was 13 agents short. Each agent now declares its
# own `OFFERS` in its crew file (or `offers` in its JSON doc) and this is DERIVED from those.
#
# Resolved LAZILY through the module __getattr__ below, never at import: the crews/ directory is
# present when the fleet runs from the repo but not when crewaimeat is installed as a library
# elsewhere, and an import-time read would bake whatever the cwd happened to be into the module.


def crew_offers(agent: str) -> list[dict]:
    """The authored offer metas this agent declares, or [] when it declares none."""
    return _crew_offers_map().get(agent, [])


def crew_offer_agents() -> tuple[str, ...]:
    """Every agent that declares at least one authored offer."""
    return tuple(_crew_offers_map())
...
def __getattr__(name: str):
    """`_CREW_OFFERS` and `CREW_AGENTS` stay readable as module attributes (PEP 562), so every existing
    caller keeps working — they are just computed on first access instead of frozen at import."""
    if name == "_CREW_OFFERS":
        return _crew_offers_map()
    if name == "CREW_AGENTS":
        return tuple(_crew_offers_map())
...
# CREW_AGENTS is served by the module __getattr__ above — derived from what the crews declare, not
# restated here. Assigning it at import would shadow that and freeze the answer to import-time cwd.
...
def publish_all(with_samples: bool = True) -> dict:
    agents = dict.fromkeys(PILOT_AGENTS + crew_offer_agents())
