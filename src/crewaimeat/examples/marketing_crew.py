...
the scaffold provides the AIMEAT wiring (see SCAFFOLD_CANON.md). Register first:
  aimeat connect --url https://aimeat.io --owner <owner> --agent marketing-crew
...
def build_domain(ctx: BuildContext) -> tuple[list[Agent], list[Task]]:
    ...
    plan_task = Task(
        description="Combine the research brief and the strategy into a single, polished marketing plan. Include: an executive summary, positioning, prioritized target audience, channel plan, messaging (pillars plus sample copy/headlines for the top channels), a phased rollout timeline with key activities, and a KPI section that lists 5-8 measurable success metrics with concrete target ranges and how each will be tracked. Make it concrete enough that a team could execute it directly. Write in clear, professional prose with clear section headings."
        + f"\n\nRequest:\n{ctx.prompt}",
