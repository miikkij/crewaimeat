...
So this module is the check, and it runs in two places that between them leave no gap:
  * `start_fleet` runs `--install`: a stopped fleet is upgraded to npm latest before the serve daemon
    starts, so the daemon that comes up IS the newest one. A LIVE daemon — this checkout's or any
    other home's on the machine, since they share the one global install — is never upgraded underneath
...
def live_serve_pids() -> list[int]:
    """EVERY `aimeat connect serve` on this machine, not only this checkout's. The global install is one
    directory shared by every home — a sibling checkout's fleet, the agency desktop — so a daemon of any
    of them would load modules from the files npm rewrites. Measured 2026-09-18: two daemons live, one of
    them another home's, which a this-home-only check would have upgraded underneath."""
    from crewaimeat.serve_guard import _serve_pids

    return list(_serve_pids())
...
def install(version: str) -> tuple[bool, str]:
    ...
    if pids:
        return False, (
            f"serve daemon(s) running on this machine (pid {', '.join(map(str, pids))}); npm would rewrite the "
            "files they load from. Stop them (scripts/terminate_fleet here; another checkout's or the "
            "desktop app's own stop for theirs), then start the fleet again to upgrade"
        )
