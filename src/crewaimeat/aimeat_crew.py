    ...
    if spec.dm_serviceable and _on_dm is None:
        ...
        def _service_responder(event: dict):
            ...
            try:
                ...
                # Ledger attribution for this DM kickoff is applied centrally in dm.handle_dm_event
                # (it wraps the responder call in usage_run("dm-<id>", agent) — covers this bridge AND
                # every on_dm crew in one place), so no usage_run wrapper is needed here.
