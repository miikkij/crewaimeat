...
STRINGS: dict[str, dict[str, str]] = {
    ...
    # test tab
    "test.placeholder": {"en": "type a test prompt, press Enter to run against this agent",
                         "fi": "kirjoita testikehote, Enter ajaa sen tälle agentille"},
    "test.idle": {"en": "Select a running agent, type a prompt below and press Enter. A real task is "
    ...
    "test.not_running": {"en": "Agent '{agent}' is not running — start it first (s).",
                         "fi": "Agentti '{agent}' ei ole ajossa — käynnistä se ensin (s)."},
    "test.busy": {"en": "A test is already running — wait for it to finish.",
                  "fi": "Testi on jo käynnissä — odota että se valmistuu."},
    "test.running": {"en": "▶ testing {agent}…", "fi": "▶ testataan {agent}…"},
    "test.done": {"en": "✓ {agent} responded in {secs}s — task {tid}",
    ...
    "mp.hint": {"en": "[b]enter[/] set + restart   [b]esc[/] cancel   ↑/↓ move",
                "fi": "[b]enter[/] aseta + uudelleen   [b]esc[/] peru   ↑/↓ liiku"},
    "mp.clear": {"en": "✗ clear override (revert to llm_providers.json routing)",
    ...
    "mp.cleared": {"en": "Override cleared for {agent}; restarting…",
                   "fi": "Ohitus poistettu agentilta {agent}; käynnistetään…"},
    "warn.no_models": {"en": "No llm_providers.json models to choose from.",
