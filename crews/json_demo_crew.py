"""json-demo — a JSON agent. Its crew lives ON THE NODE, not in this repo.

There is no definition here and there is not meant to be one: this file only names the agent. The
crew is at `crews.registry.json-demo` and you edit it in AIMEAT under
profile > agents > json-demo > Crew. Publish, and the NEXT TASK uses it — no restart, because the
...
Run standalone: uv run python crews/json_demo_crew.py
"""
