...
def build_domain(ctx: BuildContext) -> tuple[list[Agent], list[Task]]:
    ...
    verify = Task(
        description=(
            ...
            "exercise the CORE feature using your app's REAL selectors — e.g. fill an input, click the action "
            "button, then expect_text the resulting change (and wait_enabled for any control that unlocks after "
            "an async event). Fix the app and re-run until INTERACTION PASS.\n"
            ...
            "and re-run until ANON VERIFY PASS.\n"
            "   The build is not done until verify_render PASSES, AND verify_interaction PASSES (interactive "
