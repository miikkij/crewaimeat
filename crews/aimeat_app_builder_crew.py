...
def build_domain(ctx: BuildContext) -> tuple[list[Agent], list[Task]]:
    ...
    verify = Task(
        description=(
            ...
            "an async event). Fix the app and re-run until INTERACTION PASS. The build is not done until BOTH "
            "verify_render and (for interactive apps) verify_interaction PASS.\n"
