...
- **Declare what the agent IS, in the same file** — these are module-level constants beside `AGENT_NAME`, and they are the ONLY place this data lives (there is no central registry):
  - `LLM_PROFILE = "<profile>"` — which profile in `llm_providers.json` routes it. The example file carries the five this repo's crews use: `content` (English prose), `news` (Finnish prose), `coding`, `content-free` (the cheap default) and `image`. Name a profile that exists in the user's file. A crew that declares none falls to the file's `default` silently, so write it down even when the default is what you want.
  ...
  - `MODE = "..."` — only if the crew must NOT be a task-runner (e.g. every task should wait for my Start). Registration asks the node for it; leave it out otherwise.
