...
def test_the_definition_comes_from_the_node_with_its_revision(node):
    ...
    assert doc == DOC_V1 and revision == 1
...
def test_a_new_revision_is_picked_up_on_the_next_build(node):
    ...
    live = ja.Definition("node-agent", DOC_V1, 1)
    node["value"] = _envelope(DOC_V2, revision=2)

    assert live.refresh() == DOC_V2
    assert live.revision == 2
    reported = [w for w in node["writes"] if w["tool"] == "aimeat_memory_write"]
    assert reported and reported[-1]["value"]["revision"] == 2 and reported[-1]["value"]["ok"] is True
...
def test_a_broken_publish_does_not_take_the_agent_down(node, capsys):
    ...
    node["value"] = _envelope({**DOC_V1, "temperature": 9}, revision=2)
    live = ja.Definition("node-agent", DOC_V1, 1)
    ...
    assert status["ok"] is False and status["revision"] == 1
