...
def _full_edition(
    ...
    if frontpage_date:
        ...
        mem["newspaper.frontpage"] = [
            {"date": frontpage_date, "category": "talous", "title": "x"},
...
def test_exists_nonempty_count():
    ...
    assert (
        check_signal({"kind": "deterministic", "key": "news.{date}.{edition}.raw.b", "check": "nonempty"}, VARS, L)[0]
        is False
    )
    ok, obs = check_signal(
        {"kind": "deterministic", "key_glob": "news.{date}.{edition}.raw.*", "check": "count_nonempty", "min": 2},
        VARS,
        L,
    )
...
def test_json_array_match():
    mem = {
        "newspaper.frontpage": [{"date": "2026-06-11", "category": "talous"}, {"date": "2026-06-10", "category": "old"}]
    }
    L = lister_from(mem)
    ok, obs = check_signal(
        {
            "kind": "deterministic",
            "key": "newspaper.frontpage",
            "check": "json_array_match",
            "where_field": "date",
            "where_equals": "{date}",
            "min": 1,
        },
...
def test_json_field_min_and_equals():
    mem = {"news.2026-06-11.evening.quiz": {"questions": [1, 2, 3]}, "newspaper.frontpage": {"date": "2026-06-11"}}
    L = lister_from(mem)
    assert check_signal(
        {
            "kind": "deterministic",
            "key": "news.{date}.{edition}.quiz",
            "check": "json_field",
            "path": "questions",
            "min": 3,
        },
...
def test_check_workflow_red_at_editorial_output():
    ...
    res = check_workflow(
        "laimeat-sanomat-evening", {"date": "2026-06-11", "edition": "evening"}, lister=lister_from(mem)
    )
...
def test_check_workflow_empty_is_input_red_downstream():
    res = check_workflow(
        "laimeat-sanomat-evening", {"date": "2026-06-99", "edition": "evening"}, lister=lister_from({})
    )
    by = {s["id"]: s["state"] for s in res["steps"]}
